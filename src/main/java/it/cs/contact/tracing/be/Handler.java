...
import it.cs.contact.tracing.be.repository.PositiveContactsRepository;
...
public class Handler implements RequestHandler<APIGatewayProxyRequestEvent, LambdaResponse> {
	...
	final PositiveContactsRepository positiveContactsRepository = new PositiveContactsRepository();
	...
	private LambdaResponse handlePost(final APIGatewayProxyRequestEvent event) {
		...
		return LambdaResponse.builder().statusCode(positiveContactsRepository.create(body) ? HttpStatus.SC_CREATED :
	...
	private LambdaResponse handleGet(final APIGatewayProxyRequestEvent event) {
		...
		final Contact positiveContactEntity =
				getFirst(positiveContactsRepository.getPositiveContactByDeviceKey(key)).orElse(new Contact());

		return LambdaResponse.builder().statusCode(HttpStatus.SC_OK).body(gson.toJson(positiveContactEntity)).build();
	}
}